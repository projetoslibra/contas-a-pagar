# app.py — Contas a Pagar (Streamlit + SQLite)
# -------------------------------------------------
# Como rodar:
...
DB_PATH = "contas_a_pagar.db"

# 🔧 Lista fixa de empresas do grupo (edite aqui)
EMPRESAS = [
...
def init_db():
    with closing(get_conn()) as conn:
        ...
        # Garante que a coluna 'arquivo' exista (migração leve)
        cur.execute("PRAGMA table_info(boletos);")
        cols = [r[1] for r in cur.fetchall()]
        if 'arquivo' not in cols:
            cur.execute("ALTER TABLE boletos ADD COLUMN arquivo TEXT;")
        conn.commit()
    ...
    with closing(get_conn()) as conn:
        ...
        cur.execute(
            ...
            INSERT INTO boletos(
                ...
                beneficiario, banco, setor, status, data_pagamento, obs, arquivo
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
...
def fetch_boletos(filters: dict | None = None) -> pd.DataFrame:
    ...
    if filters:
        if empresa_list := filters.get("empresa_list"):
            q_marks = ",".join(["?"] * len(empresa_list))
            base_sql += f" AND empresa_pagadora IN ({q_marks})"
            params.extend(empresa_list)
        if beneficiario_list := filters.get("beneficiario_list"):
            q_marks = ",".join(["?"] * len(beneficiario_list))
            base_sql += f" AND beneficiario IN ({q_marks})"
            params.extend(beneficiario_list)
        if setor_list := filters.get("setor_list"):
            q_marks = ",".join(["?"] * len(setor_list))
            base_sql += f" AND setor IN ({q_marks})"
            params.extend(setor_list)
        if status_list := filters.get("status_list"):
            q_marks = ",".join(["?"] * len(status_list))
            base_sql += f" AND status IN ({q_marks})"
            params.extend(status_list)
        if dt_ini := filters.get("dt_ini"):
            base_sql += " AND date(data_vencimento) >= date(?)"
    ...
    with closing(get_conn()) as conn:
        ...
        for i in edited_rows:
            ...
            cur.execute(
                """
                UPDATE boletos SET
                    data_registro = ?, empresa_pagadora = ?, valor = ?,
                    data_vencimento = ?, beneficiario = ?, banco = ?, setor = ?,
                    status = ?, data_pagamento = ?, obs = ?, arquivo = ?
                WHERE id = ?
                """,
                (
...
def delete_by_ids(ids: list[int]):
    ...
    with closing(get_conn()) as conn:
        ...
        cur.execute(f"DELETE FROM boletos WHERE id IN ({q_marks})", ids)
        conn.commit()
...
if page == "Adicionar Boleto":
    ...
    with st.form("form_boleto", clear_on_submit=True):
        ...
        if submitted:
            arquivo_path = None
            if pdf is not None:
                UP_DIR = "uploads/boletos"
                os.makedirs(UP_DIR, exist_ok=True)
                safe_benef = (beneficiario or "boleto").replace("/", "-").replace("\\", "-")
                fname = f"{date.today().isoformat()}_{uuid.uuid4().hex[:8]}_{safe_benef}.pdf"
                full_path = os.path.join(UP_DIR, fname)
                with open(full_path, "wb") as f:
                    f.write(pdf.read())
                arquivo_path = full_path
            ...
            if not payload["empresa_pagadora"]:
                ...
                force_rerun()  # 🔁 recarrega após inserir
...
# =============================
    ...
    with st.expander("🔎 Filtros", expanded=True):
        ...
        # usa sempre a lista fixa para padronização:
        emp_opts = EMPRESAS
    ...
    if not df_edit.empty:
        ...
        cfg = {
            ...
            "arquivo": st.column_config.TextColumn("Arquivo (local)", help="Caminho do PDF salvo"),
        ...
        with colb1:
            if st.button("💾 Salvar alterações", disabled=(len(changed_rows) == 0)):
                ...
                force_rerun()  # 🔁 recarrega após salvar
        with colb2:
            ...
            if st.button("🗑️ Excluir selecionados", disabled=(len(ids_to_delete) == 0)):
                ...
                force_rerun()  # 🔁 recarrega após excluir
    ...
    if file is not None:
        ...
        if any(c is None for c in required):
            ...
            force_rerun()  # 🔁 recarrega após importar
    ...
    if df.empty or df["arquivo"].dropna().empty:
        st.caption("Sem anexos para os filtros atuais.")
    else:
        anexos = df.dropna(subset=["arquivo"]).copy()
        for _, r in anexos.iterrows():
            try:
                ...
                colD.download_button(
                    "⬇️ Baixar PDF",
                    data=pdf_bytes,
                    file_name=f"boleto_{int(r['id'])}.pdf",
                    mime="application/pdf",
                    key=f"dl_{int(r['id'])}"
                )
            except Exception:
                st.warning(f"Não foi possível ler o arquivo do boleto ID {int(r['id'])}.")
